...
class ChatConsumer(WebsocketConsumer):
    ...
    def connect(self):
        self.accept()
        self.send(text_data=json.dumps({
            'message': 'rec//1//오늘의 야구 경기를 시작하겠습니다!!',
        }))
    ...
    def inning_proc(self, homePitchers, homeBatters, awayPitchers, awayBatters):  # 경기 진행 로직
        ...
        innings = range(1, 13)  # 이닝(1 ~ 12)   테스트용은 3회까지
        ...
        for inning in innings:  # 9 ~ 12이닝
            for top_and_bottom in range(1, 3):  # 이닝별 초, 말
                ...
                if top_and_bottom == 1:  # 이닝 초
                    ...
                    self.record_send('rec' + '//' + str(inning) + '//' + str(inning) + '회 초 시작')

                    out = 0
                    self.record_send('rec' + '//' + str(inning) + '//' + str(inning) + '회 초 원정팀 공격')
                    ...
                    while True:  # 3아웃 되는 동안 실행
                        if home_pitcher == home_rp and (
                            ...
                            self.record_send('rec' + '//' + str(inning) + '//' + 'cp로 투수 교체')
                            ...
                            self.record_send('rec' + '//' + str(inning) + '//' + 'rp로 투수 교체')
                        ...
                        if game.main(home_pitcher[7], away_hitter_num[11]):  # 투수 VS 타자가 아웃일 경우
                            ...
                            self.record_send('rec' + '//' + str(inning) + '//' + str(away_hitterIndex+1) + '번 타자, ' + str(o_res))
                            if home_pitcher == home_sp:  # 투수가 선발투수일 시 투구 수 추가
                                if o_res == '삼진':
                                    home_sp_pitch_count += game.pitch_count_case_K()
                                    self.record_send('rec' + '//' + str(inning) + '//' + '투구 수 : ' + str(home_sp_pitch_count))
                                else:
                                ...
                                if o_res == '삼진':
                                    ...
                                    self.record_send('rec' + '//' + str(inning) + '//' + '투구 수 : ' + str(home_rp_pitch_count))
                                else:
                                ...
                                if o_res == '삼진':
                                    ...
                                    self.record_send('rec' + '//' + str(inning) + '//' + '투구 수 : ' + str(home_cp_pitch_count))
                                else:
                            ...
                            if res != '볼넷':     # 안타일 경우
                                ...
                                away_hit += 1
                                self.record_send('score' + '//' + 'away' + '//' + 'hit' + '//' + str(away_hit))
                                ...
                                if home_pitcher == home_rp:  # 홈팀 투수 중계투수일 경우
                                    home_rp_scored += 1  # 홈팀 중계투수 실점 + 1
                                away_bb += 1
                                self.record_send('score' + '//' + 'away' + '//' + 'bb' + '//' + str(away_bb))

                            away_hitterIndex += 1  # 타자번호 + 1
                            if away_hitterIndex == 9:  # 9번타자 다음은 1번타자
                                away_hitterIndex = 0

                            base_occ = game.printOccupiedBase(base)


                            self.record_send('rec' + '//' + str(inning) + '//' + str(away_hitterIndex + 1) + '번 타자,' + str(res))
                            self.record_send('rec' + '//' + str(inning) + '//' + ','.join(base_occ) +
                        ...
                        self.record_send('rec' + '//' + str(inning) + '//' + str(out) + '아웃')

                    self.record_send('rec' + '//' + str(inning) + '//' + '3 아웃 change')  # 3아웃 됐을 시, 베이스 초기화
                    self.record_send('rec' + '//' + str(inning) + '//' + str(inning) + '회 초 종료,' + '원정' +
                                     str(game.away_score) + ':' + str(game.home_score) + '홈')
                    self.record_send('rec' + '//' + str(inning) + '//' + '-------------------------------------------------------------------')
                    base = [0, 0, 0]
                    away_score_list = [inning, game.inning_away_score]
                    # self.record_send('어웨이팀,' + '이닝,' + '점수' + str(away_score_list))
                    self.record_send('score' + '//' + 'away' + '//' + str(inning) + '//' + str(game.inning_away_score) + '//' + str(game.away_score))
                    game.inning_away_score = 0
                    if inning > 8 and game.home_score > game.away_score:  # 9회 이상일때, 홈팀 점수가 어웨이 보다 높으면 종료
                        break
                    else:
                        self.record_send('rec' + '//' + str(inning) + '//' + str(inning) + '회 말 홈팀 공격')
                else:
                    out = 0
                    while True:
                        ...
                        if game.main(away_pitcher[7], home_hitter_num[11]):
                            ...
                            self.record_send('rec' + '//' + str(inning) + '//' + str(home_hitterIndex + 1) + '번 타자, ' + o_res)
                            if away_pitcher == away_sp:
                                if o_res == '삼진':
                                    away_sp_pitch_count += game.pitch_count_case_K()
                                    self.record_send('rec' + '//' + str(inning) + '//' + '투구 수 : ' + str(away_sp_pitch_count))
                                else:
                                ...
                                if o_res == '삼진':
                                    ...
                                    self.record_send('rec' + '//' + str(inning) + '//' + '투구 수 : ' + str(away_rp_pitch_count))
                                else:
                                ...
                                if o_res == '삼진':
                                    ...
                                    self.record_send('rec' + '//' + str(inning) + '//' + '투구 수 : ' + str(away_cp_pitch_count))
                                else:
                            ...
                            if res != '볼넷':
                                ...
                                if away_pitcher == away_sp:  # 어웨이팀 투수 선발투수일 경우
                                    away_sp_scored += 1  # 어웨이팀 선발투수 실점 + 1
                                if away_pitcher == away_rp:  # 어웨이팀 투수 중계투수일 경우
                                    away_rp_scored += 1  # 어웨이팀 중계투수 실점 + 1
                                home_hit += 1
                                self.record_send('score' + '//' + 'home' + '//' + 'hit' + '//' + str(home_hit))
                                ...
                                if away_pitcher == away_rp:  # 어웨이팀 투수 중계투수일 경우
                                    away_rp_scored += 1  # 어웨이팀 중계투수 실점 + 1
                                home_bb += 1
                                self.record_send('score' + '//' + 'home' + '//' + 'bb' + '//' + str(home_bb))

                            base_occ = game.printOccupiedBase(base)

                            self.record_send('rec' + '//' + str(inning) + '//' + str(home_hitterIndex) + '번 타자 ' + res)
                            self.record_send('rec' + '//' + str(inning) + '//' + ','.join(base_occ) +
                                             '원정' + str(game.away_score) + ':' + str(game.home_score) + '홈')
                            if away_pitcher == away_sp:
                    ...
                    self.record_send('rec' + '//' + str(inning) + '//' + '3 아웃 change')
                    self.record_send('rec' + '//' + str(inning) + '//' + str(inning) + '회 말 종료,' + '원정 ' + str(game.away_score)
                    ...
                    self.record_send('score' + '//' + 'home' + '//' + str(inning) + '//' + str(game.inning_home_score) + '//' + str(game.home_score))
        ...
        self.record_send('rec' + '//' + str(inning) + '//' + '경기 종료')
        if game.home_score > game.away_score:
            self.record_send('rec' + '//' + str(inning) + '//' + '홈팀 승리')
        elif game.away_score > game.home_score:
            self.record_send('rec' + '//' + str(inning) + '//' + '원정팀 승리')
        else:
            self.record_send('rec' + '//' + str(inning) + '//' + '무승부')
